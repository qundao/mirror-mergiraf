...
mod common;
use common::{
    DEFAULT_FILE_FOR_SOLVE, DEFAULT_FILE_FOR_SOLVE_SOLVED, create_file_for_solve,
    create_files_for_merge, merge, solve,
};
...
fn allow_in_jj() {
    ...
    // imagine that, for some reason, this file has Git-style conflicts,
    // and so we want to run `mergiraf solve` on it
